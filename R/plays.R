...
add_score_events = function(data, td_text = "TD$", fg_text = "FG$", safety_text = "SF$") {
    ...
    data |>
        ...
        mutate(
            last_drive_in_half = case_when(drive_number == max(drive_number) ~ T,
                                           TRUE ~ F)
        ) |>
        # fill in next score
        group_by(game_id) |>
        mutate(
            next_score_event = case_when(is.na(score_event) & last_drive_in_half == T ~ 'No_Score',
                                         TRUE ~ score_event)
        ) |>
        ...
        mutate(next_score_event_offense = case_when(drive_is_home_offense == T & next_score_event == 'HOME TD' ~ 'TD',
